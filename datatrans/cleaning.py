...
def remove_url(strng):
    ...
    REGEXEN['valid_domain'] = re.compile(
        '(?:[^%s\s][\.-](?=[^%s\s])|[^%s\s]){1,}\.[a-z]{2,}(?::[0-9]+)?' % (punct, punct, punct), re.IGNORECASE)
...
def cleanThis(strng):
    ...
    stopwords = ["a", "about", "above", "above", "across", "after", "afterwards", "again", "against", "all", "almost",
                 ...
                 "might", "mill", "mine", "more", "moreover", "most", "mostly", "move", "much", "must", "namely",
    ...
    for word in str_array:
        ...
        while i < length and not neglect:
            if word[i] == "'":
                i = i + 1
            elif word[i] == "$" or word[i] == ":" or word[i] == ";" or ord(word[i]) - ord('0') >= 0 and ord(
                    word[i]) - ord('9') <= 0:
                neglect = True
...
# no_of_tweets = 10
lmt = 100000
no_of_tweets = db.Labeled_Tweets.count_documents()#find().count()
